...
Patient encounters can be duplicated due to BioSense's processing rules (encounter can be duplicated based on patient's arrival time and the time update messages were sent (e.g., patient arrives at 10PM, update message is sent at 2 AM the following day)), or by facility-side data quality issues (e.g., facility changing a patient's assigned medical record number (MRN) mid-visit, so initial message comes in with one MRN value, and a different MRN value is assigned mid-encounter and is sent with all subsequent message updates to that encounter).
