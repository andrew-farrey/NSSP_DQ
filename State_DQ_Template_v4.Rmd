...
description: "This is a state data quality report that uses your site's PR_Processed table and the ESSENCE API. Please knit with parameters within the NSSP RStudio Workbench. This report will not work in other instances of RStudio without considerable modification."
output:
  html_document:
    toc: true
params:
...
# Note: packages are called explicitly when a known conflict exists that's caused an issue before (e.g., plotly has a select() function that supersedes dplyr::select() since it's loaded after dplyr) 
```
...
# Create time for display & calculate number of hours to offset Arrived_Date_Time by based on user's local time zone
...
# Pull NSSP Priority 1 & Priority 2 elements, and timeliness elements
# Note: pull can be adjusted to use Patient_Class_Code or Facility_Type_Code, but I prefer C_Patient_Class as a QI measure so one can tell if facilities are sending visits of a particular patient class that they shouldn't be and to mirror production ESSENCE as closely as possible. 
messages <-
  RODBC::sqlQuery(
    con,
    paste0(
...
messages <- messages %>%
  ...
  mutate(
    ...
    Feed = paste0(params$site,"_Feed")
  ) %>%
  mutate(Arrived_Date_Time = Arrived_Date_Time - lubridate::hours(paste0(adt_adjustment))) %>%
  #filter(!C_Biosense_Facility_ID %in% c(15466, etc.)) %>% # remove the leftmost number sign and add the C_Biosense_Facility IDs you want to omit
...
This report is intended to be used to monitor state syndromic surveillance data quality by National Syndromic Surveillance Program (NSSP) participants.

## Introduction

This report pulls and analyzes de-identified syndromic patient encounter data from the `r paste(params$site)`\_PR_Processed table in the NSSP BioSense syndromic surveillance platform and the ESSENCE API, restricted to `r paste(pat.class)` patient encounters from active, NSSP-participating facilities occurring on/between **`r start_date_lab`** through **`r end_date_lab`**. The `r paste(params$site)`\_PR_Processed table populates the NSSP ER_Base table, which in turn populates NSSP BioSense ESSENCE, the national syndromic surveillance web platform. Please note the patient encounter dates analyzed include a built in "grace period" of 24 hours.[^1]
...
Each patient encounter (or "patient visit") reported through syndromic surveillance consists of an initial HL7 message followed by additional HL7 messages that contain updates to message fields from that patient encounter. These updated messages can be sent within minutes or hours, but typically are sent within several days or weeks.

Facilities vary in how quickly they send completed patient encounter information, so NSSP assesses and monitors timeliness criteria to encourage timely data submission. Overall, or "first message" timeliness is calculated on a per patient encounter basis using the **earliest**, or **first message** received from each patient encounter. In order for a per patient metric to as useful as possible, mean or median first message timeliness either by site or by facility is typically reported.

First message timeliness is calculated specifically by comparing the time difference in hours between **C_Visit_Date_Time,**[^2] and the **first/earliest** **Arrived_Date_Time**[^3] of the first message received for each unique patient encounter, where C_Visit_Date_Time represents the date-time of the physical patient encounter and Arrived_Date_Time represents the date-time NSSP received the first message from that patient encounter at the BioSense platform.
...
The following plots include analysis of `r paste(params$site)`'s site-wide syndromic timeliness (`r paste(pat.class)` only), and counts and percentage of visits by NSSP timeliness category (by facility).
...
The following plot displays `r paste(params$site)`'s site-wide percentages for each NSSP timeliness category from `r start_date_lab` through `r end_date_lab`.
...
This plot display patient visit counts by timeliness category for each facility from `r start_date_lab` through `r end_date_lab`.[^4]
...
This plot display the percentage of patient visits for each timeliness category by facility from `r start_date_lab` through `r end_date_lab`.
...
NSSP has stipulated that chief complaint information should also be received by NSSP within 24 hours of each patient encounter (and not more than 48 hours after). Chief complaint timeliness is a field-specific delay time calculated by comparing the C_Visit_Date_Time to the Arrived_Date_Time of the first HL7 message with a non-null chief complaint field per patient encounter. While not an official NSSP timeliness metric, this metric is useful for ensuring timely submission of a useful (and necessary) syndromic surveillance data field.
...
# Calculate average and median lag for the first message with a non-null chief complaint field by facility
...
NSSP has also stipulated that diagnosis code information should begin to arrive at NSSP within 24 hours of each patient encounter (and not more than 48 hours after). Diagnosis code timeliness is a field-specific delay time calculated by comparing the C_Visit_Date_Time to the Arrived_Date_Time of the first HL7 message with a non-null diagnosis code field per patient encounter. Like chief complaint timeliness, diagnosis code timeliness is not an official NSSP timeliness metric, but this metric is useful for ensuring timely submission of a useful (and necessary) syndromic surveillance data field.
...
# Calculate average and median lag for first message with a non-null diagnosis code by facility
...
NSSP requires 90% completeness for these Priority 1 and Priority 2 elements across each patient encounter. In other words, at least 90% of patient encounters should have at least one message where each required element is not null to ensure useful syndromic surveillance data.

Completeness is calculated on a per message field basis, as the number of patient encounters with a completed, non-null value for that message field over the total number of unique patient encounters.

```{r completeness_metrics, echo=FALSE, warning=FALSE, message=FALSE, fig.align='center', fig.width=10, fig.height=8}
...
## Chief Complaint and Diagnosis Code Completeness by Facility Table, Since `r start_date_lab` (From `r paste(params$site)`\_PR_Processed)

The following table details percent completeness of the chief complaint field (C_Chief_Complaint), diagnosis code (Diagnosis_Code) field, and counts of `r paste(pat.class)` visits with missing chief complaint and diagnosis code fields, by `r paste(params$site)` facility. The table is sorted by lowest DX code percent completeness to highest, and depicts an aggregated metric of chief complaint and diagnosis code completeness for the time period analyzed, by facility..

Note that chief complaint and diagnosis code completeness are measured across the entire patient encounter, not the individual HL7 message.
...
Green indicates greater than or equal to 90% completeness (NSSP's percent completeness requirement for required patient encounter fields), orange indicates percent completeness between 60%-90%, and red indicates percent completeness below 60% for that facility and metric.
...
NSSP Priority 1 and Priority 2 element validity metrics are presented in the plot and facility-specific tables below (including hyperlinks to all relevant code sets), followed by a plot and table of patient encounter messages sent to the Exceptions table due to either a missing required data element or an inconsistency with the `r paste(params$site)`\_Operational_Crosswalk or `r paste(params$site)`\_MFT (Master Facility Table).

## Overall `r paste(params$site)` Validity (`r paste(params$site)`\_PR_Processed Table)
...
```{r validity_plot, echo=FALSE,warning=FALSE,message=FALSE,fig.align='center',fig.width=10,fig.height=8}
...
## Validity by Message Field by Facility (`r paste(params$site)`\_PR_Processed Table)

The tables below display `r paste(params$site)`'s percent validity across NSSP Priority 1 and Priority 2 data quality elements by facility, for messages from all `r paste(pat.class)` patient encounters received by the `r paste(params$site)`\_PR_Processed table between `r start_date_lab` and `r end_date_lab` (as of `r local_time_display`).
...
-   The DDInformative field measures whether a given patient encounter has an informative 'discharge_diagnosis' (the DD of the CCDD, populated by the Diagnosis_Code field from the `r paste(params$site)`\_PR_Processed table) element at the ESSENCE patient encounter level after non-informative chief complaint (NICC) terms, the phrase "null," and blank spaces have been removed.
